...
9. **CALENDAR INTEGRATION (MANDATORY if calendar tools are available)**: 
   If Google Calendar integration is enabled, create a calendar event with these EXACT details:
   - **Calendar**: Use the primary calendar (ramyasruthi.pedakolimi@gmail.com)
   - **Title**: "Learning Path: [user_goal]"  
   - **Description**: "Generated learning path for: [user_goal]. Complete this learning path in [X] days."
   - **Start Date & Time**: Today's date + 1 day at 9:00 AM in format YYYY-MM-DD 09:00:00
   - **End Date & Time**: Start date + number of learning path days at 6:00 PM in format YYYY-MM-DD 18:00:00
   - **Time Zone**: America/New_York (or user's local timezone if known)
   - **Event Type**: All-day event set to false
   
   **EXAMPLE**: If creating a 5-day learning path on 2025-01-15:
   - Start: 2025-01-16 09:00:00
   - End: 2025-01-20 18:00:00
   - Calendar: ramyasruthi.pedakolimi@gmail.com
   
   **You MUST provide specific dates and times. Do not use relative terms like "tomorrow" - calculate actual dates.**
...
8. **CALENDAR CHECK**: Always check if calendar tools are available and use them if present.
