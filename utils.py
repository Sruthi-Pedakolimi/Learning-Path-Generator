...
async def setup_agent_with_tools(
    ...
    google_calendar_pipedream_url: Optional[str] = None,
    ...
    try:
        ...
        if google_calendar_pipedream_url:
            tools_config["google_calendar"] = {
                "url": google_calendar_pipedream_url,
                "transport": "streamable_http"
            }
            if progress_callback:
                progress_callback("Added Google Calendar integration... ✅")
        ...
        tools = await mcp_client.get_tools()
         ...
         # DEBUG: Print available tools to check if calendar tools are loaded
        print(f"Available tools: {[tool.name for tool in tools]}")
        calendar_tools = [tool for tool in tools if 'calendar' in tool.name.lower()]
        print(f"Calendar tools found: {[tool.name for tool in calendar_tools]}")

        if google_calendar_pipedream_url and not calendar_tools:
            print("WARNING: Google Calendar URL provided but no calendar tools found!")
    ...
    async def _run():
        try:
            agent = await setup_agent_with_tools(
                ...
                google_calendar_pipedream_url=google_calendar_pipedream_url,
                progress_callback=progress_callback
            )
            
            # Enhanced prompt with calendar integration check and current date
            from datetime import datetime
            current_date = datetime.now().strftime("%Y-%m-%d")

            calendar_instruction = ""
            if google_calendar_pipedream_url:
                calendar_instruction = f"\n\nIMPORTANT: Google Calendar integration is ENABLED. Current date is {current_date}. You MUST create a calendar event as specified in step 9 of the instructions with specific dates and times."
            # Combine user goal with prompt template
            learning_path_prompt = ("User Goal: " + user_goal + "\n" + user_goal_prompt + calendar_instruction)
